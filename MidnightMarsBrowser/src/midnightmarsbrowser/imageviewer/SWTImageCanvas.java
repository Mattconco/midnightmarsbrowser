package midnightmarsbrowser.imageviewer;
...
public class SWTImageCanvas extends Canvas {
	...
	public SWTImageCanvas(final Composite parent, int style) {
		...
		// Initially scale so that a 1024x1024 image is fully displayed
/*		Rectangle destRect = getClientArea();
		double sx = (double) destRect.width / (double) 1024;
		double sy = (double) destRect.height / (double) 1024;
	...
	public Image loadImage(String filename) {
		try {
			...
			if (filename.toUpperCase().endsWith(".IMG")) {
				PDSIMG img = PDSIMG.readIMGFile(filename, pdsBrightnessMinValue, pdsBrightnessMaxValue);
	 ...
	 * Fit the image onto the canvas
	 */
	public void fitCanvas1024() {
		Rectangle destRect = getClientArea();
		double sx = ((double) destRect.width) / 1024.0;
		double sy = ((double) destRect.height) / 1024.0;
		double s = Math.min(sx, sy);
		double dx = 0.5 * destRect.width;
		double dy = 0.5 * destRect.height;
		centerZoom(dx, dy, s, new AffineTransform());
